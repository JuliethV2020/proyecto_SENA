...
if(empty($busqueda))
{
    header ("localhost:Listar_mesa.php");
...
<?php include "includes/header.php";?>
    ...
    <table>
        <tr>
         ...
         <?php 
         //paginador
         $sql_register=mysqli_query($conexion,"SELECT COUNT(*) as total_registro FROM mesa WHERE (id_mesa LIKE '%$busqueda%' OR 
         numero LIKE '%$busqueda%' OR 
        ubicacion LIKE '%$busqueda%')
         AND
         estatus=1");
         ...
         $query=mysqli_query($conexion,"SELECT id_mesa,numero,ubicacion,foto FROM mesa WHERE 
         
         (id_mesa LIKE '%$busqueda%' OR
          numero LIKE '%$busqueda%' OR 
          ubicacion LIKE '%$busqueda%' )
         AND
         estatus=1 ORDER BY id_mesa ASC LIMIT $desde,$por_pagina");
